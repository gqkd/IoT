...
if __name__ == '__main__':
    ...
    # Temperatura:
    temp1 = SensorTemperature("100", "002", topic, publicURL)
    # Accelerazione:
    acc1 = SensorAcceleration("200", "002", topic, publicURL)
    # Massa:
    mass1 = SensorMass("300", "002", topic, publicURL)
    # Livello di ossigenazione:
    oxy1 = SensorOxygen("400", '002', topic, publicURL)
    # Speaker
    speak1 = Speaker('500', '002', broker, port, publicURL)
    # GPS
    GPS1 = SensorGPS('600', '002', topic, publicURL)
    # Controllo accelerazione
    contAcc1 = AccelerationControl('011_Acc_controlService', topic, broker, port, publicURL)
    # Controllo temperatura
    contTemp1 = TemperatureControl('022_Temp_controlService', topic, broker, port, publicURL)
    # Controllo ossigeno
    contOx1 = OxygenControl('033_OX_controlService', topic, broker, port, publicURL)
    # Controllo massa
    contMas1 = WeightControl('044_We_controlService', topic, broker, port, publicURL)
    # Controllo stato di salute
    contHealth1 = HealthControl('055_Health_controlService', topic, broker, port, publicURL)
